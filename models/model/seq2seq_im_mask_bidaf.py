...
class Module(Base):
    ...
    def __init__(self, args, vocab):
        ...
        self.lang_emb_bidaf = nn.Sequential(nn.Linear(args.dhid*2, args.bidaf_dim), nn.ReLU())
        # subgoal monitoring
        self.subgoal_monitoring = (self.args.pm_aux_loss_wt > 0 or self.args.subgoal_aux_loss_wt > 0)

        # frame mask decoder
        decoder = vnn.ConvFrameMaskDecoderProgressMonitorBiDAF if self.subgoal_monitoring else vnn.ConvFrameMaskDecoder
    ...
    def encode_lang(self, feat):
        ...
        cont_lang_goal_instr = self.lang_emb_bidaf(cont_lang_goal_instr)
        enc_lang_goal_instr = self.lang_emb_bidaf(enc_lang_goal_instr)
