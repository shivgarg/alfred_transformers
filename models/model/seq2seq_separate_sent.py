...
class Module(Base):
    ...
    def __init__(self, args, vocab):
        ...
        transformer_demb = 768
        fc_demb = 200
        self.enc_att = vnn.SelfAttn(transformer_demb)
        self.fc = nn.Linear(transformer_demb, fc_demb)
        ...
        self.dec = decoder(self.emb_action_low, args.dframe, transformer_demb, fc_demb,
                           pframe=args.pframe,
    ...
    def featurize(self, batch, load_mask=True, load_frames=True):
        ...
        goal_list = list()
        instr_list = list()
        max_instr_len = max([ex['images'][-1]['high_idx'] for ex in batch])+2
        ...
        for ex in batch:
            ...
            r_idx = ex['repeat_idx']
            goal_str = ex['turk_annotations']['anns'][r_idx]['task_desc'] + '<<goal>>'
            instr_str = copy.deepcopy(ex['turk_annotations']['anns'][r_idx]['high_descs'])
            instr_str.append('<<stop>>')
            while(len(instr_str)!=max_instr_len):
                instr_str.append('')

            instr_list.extend(instr_str)
...
# load Resnet features from disk
            ...
            if not self.test_mode:
                ...
                feat['action_high'].append([a['high_idx'] for a in ex['num']['action_low']])
        ...
        feat['lang_instr'] = self.tokenizer(instr_list, return_tensors='pt', padding=True).to(device)
        feat['lang_goal'] = self.tokenizer(goal_list, return_tensors='pt', padding=True).to(device)
        feat['instr_len'] = max_instr_len
        return feat
    ...
    def forward(self, feat, max_decode=300):
        cont_lang, enc_instr, enc_goal = self.encode_lang(feat)
        enc_goal = F.relu(self.fc(enc_goal))
        state_0 = cont_lang, torch.zeros_like(cont_lang)
        frames = self.vis_dropout(feat['frames'])
        res = self.dec(enc_instr, enc_goal, frames, max_decode=max_decode, gold=feat['action_low'], state_0=state_0)
        feat.update(res)
        return feat
    ...
    def encode_lang(self, feat):
        ...
        '''
        lang_instr = feat['lang_instr']
        lang_goal = feat['lang_goal']
        instr_len = feat['instr_len']

        roberta_instr = self.enc(**lang_instr)
        roberta_goal = self.enc(**lang_goal)

        roberta_instr_last_layer_vec = roberta_instr.last_hidden_state
        roberta_goal_last_layer_vec = roberta_goal.last_hidden_state
        
        roberta_instr_cls = roberta_instr_last_layer_vec[:,0,:] # (BS*instr_len) X seq_len X 768
        roberta_goal_cls = roberta_goal_last_layer_vec[:,0,:] # BS X seq_len X 768
        
        self.lang_dropout(roberta_instr_cls)
        self.lang_dropout(roberta_goal_cls)

        roberta_instr_cls = roberta_instr_cls.view(-1, instr_len, 768)
        
        cont_roberta_output = self.enc_att(roberta_instr_cls)

        return cont_roberta_output, roberta_instr_cls, roberta_goal_cls
    ...
    def compute_loss(self, out, batch, feat):
        ...
        p_ahigh = out['out_attn_scores'].view(-1, feat['instr_len'])
        l_ahigh = feat['action_high'].view(-1)
 
        # action loss
        pad_valid = (l_alow != self.pad)
        ahigh_loss =  F.cross_entropy(p_ahigh, l_ahigh, reduction='none')
        ahigh_loss *= pad_valid.float()
        ahigh_loss = ahigh_loss.mean()
