...
class EvalTask(Eval):
    ...
    def evaluate(cls, env, model, r_idx, object_detector, tfms, traj_data, args, lock, successes, failures, results):
        ...
        while not done:
            ...
            x = tfms(curr_image).to('cuda')
            features = model.backbone(x.unsqueeze(0))
            predictions = model(x.unsqueeze(0))
            boxes =  [torch.cat([torch.Tensor([[0,0,75,75]]).to("cuda"),predictions[0]['boxes'][:10]/4.0])]
            rois = torchvision.ops.roi_align(features['0'],boxes,(7,7))
            feat['frames'] = torch.vstack(rois).unsqueeze(0).unsqueeze(0).to('cuda')
