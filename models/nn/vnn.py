...
class ConvFrameMaskDecoderProgressMonitorRegNet(nn.Module):
    '''
    action decoder with subgoal and progress monitoring
    '''
...
class ConvFrameMaskDecoderProgressMonitorFinetune(nn.Module):
    '''
    action decoder with subgoal and progress monitoring
    '''
    ...
    def __init__(self, emb, dframe, dhid, pframe=300,
        ...
        self.pframe = pframe
        self.dhid = dhid
        self.vis_encoder = Resnet18(dframe=dframe)
        self.cell = nn.LSTMCell(dhid+dframe+demb, dhid)
        self.attn = DotAttn()
        self.input_dropout = nn.Dropout(input_dropout)
        self.attn_dropout = nn.Dropout(attn_dropout)
